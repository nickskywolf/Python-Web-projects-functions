...
class Auth:
    ...
    red_cash = redis.Redis(host=settings.redis_host, port=settings.redis_port, db=0)
    ...
    async def decode_refresh_token(self, refresh_token: str):
        ...
        :param refresh_token: str: Pass in the refresh token that was generated when a user logged in
        :return: The email of the user who is trying to access an endpoint
        :doc-author: Trelent
        """
        try:
    ...
    async def get_current_user(self, token: str = Depends(oauth2_scheme), db: Session = Depends(get_db_func)):
        ...
        user = self.red_cash.get(f"user:{email}")
        if user is None:
            user = await repository_users.get_user_by_email(email, db)
            if user is None:
                raise credentials_exception
            self.red_cash.set(f"user:{email}", pickle.dumps(user))
            self.red_cash.expire(f"user:{email}", 900)
        else:
    ...
    def create_email_token(self, data: dict):
        ...
        :param self: Make the function a method of the class
        :param data: dict: Pass the data that will be encoded into a token
