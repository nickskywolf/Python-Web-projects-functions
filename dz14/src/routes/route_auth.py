...
from src.valid_schemas import UserModel, UserResponse, TokenModel, RequestEmail
...
async def signup(body: UserModel, background_tasks: BackgroundTasks, request: Request,
    ...
    background_tasks.add_task(send_email, new_user.email, new_user.username, request.base_url)  # mailing
...
async def confirmed_email(token: str, db: Session = Depends(get_db_func)):
    """
